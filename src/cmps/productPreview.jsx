...
export function ProductPreview({ product, layout }) {
  ...
  return (
    <Link to={`/product/${product._id}`} className='flex flex-col sm:flex-row p-7 shadow-2xl rounded-lg text-center cursor-pointer hover:scale-105 duration-300 mt-6'>
      <img className='w-40 h-40 object-cover rounded-lg m-auto sm:m-0' src={product.attributes.image} alt="" />
      <div className='sm:ml-6 sm:flex sm:flex-col sm:items-start'>
        <h1 className='mt-4 sm:mt-0 capitalize text-2xl tracking-wider font-medium'>{product.attributes.title}</h1>
        <h2 className='mt-4  capitalize text-xl tracking-wider font-medium'>{product.attributes.company}</h2>
      </div>
      <p className='ml-auto tracking-wider text-secondary'>$ {parseFloat(product.attributes.price) / 100}</p>
    </Link>
  )
}